...
func CreateTable() (error){
	...
	result, err = db.Exec("CREATE TABLE IF NOT EXISTS cliente (" + 		"id INT AUTO_INCREMENT " + 
	...
	result, err = db.Exec("CREATE TABLE IF NOT EXISTS endereco("+					"id INT AUTO_INCREMENT PRIMARY KEY, " +
						"fk_estado INT, " + 
						"cidade VARCHAR(25), " + 
						"bairro VARCHAR(25), " +
	...
	result, err = db.Exec("CREATE TABLE estado(" +
	...
	result, err = db.Exec("CREATE TABLE login(" +
							"email VARCHAR(70), " + 
							"senha VARCHAR(100), " +
							"fk_cliente INT, " +
							"token	VARCHAR(100), " +
